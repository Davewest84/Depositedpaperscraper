...
```
python scraper.py --date-from 2024-01-01 --date-to 2024-12-31
```

Also download attached files:
...
| Flag | Description |
|------|-------------|
| `--search`, `-s` | Free-text search term |
| `--date-from` | Papers deposited on or after this date (YYYY-MM-DD) |
| `--date-to` | Papers deposited on or before this date (YYYY-MM-DD) |
| `--member-id` | Filter by depositing member ID |
| `--format`, `-f` | Output format: `json`, `csv`, or `both` (default: `both`) |
| `--output-dir`, `-o` | Output directory (default: `output/`) |
| `--download-files` | Download attached files for each paper |
| `--page-size` | Results per API request (default: 20, max: 100) |
